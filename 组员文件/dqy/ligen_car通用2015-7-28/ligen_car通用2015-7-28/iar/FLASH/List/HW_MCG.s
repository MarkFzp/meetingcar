...
//    List file    =  
//        C:\Users\Administrator\Desktop\lpld\LPLD_OSKinetis_V3.1\project\ligen_car通用7-23-20.36\iar\FLASH\List\HW_MCG.s
//
///////////////////////////////////////////////////////////////////////////////
...
??LPLD_PLL_Setup_15:
        LDR.N    R0,??LPLD_PLL_Setup_16  ;; 0x4007d002
        LDRB     R0,[R0, #+0]
        LSLS     R0,R0,#+28
        BPL.N    ??LPLD_PLL_Setup_17
//  360         PMC->REGSC |= PMC_REGSC_ACKISO_MASK;
        LDR.N    R0,??LPLD_PLL_Setup_16  ;; 0x4007d002
        LDRB     R0,[R0, #+0]
        ORRS     R0,R0,#0x8
        LDR.W    R12,??LPLD_PLL_Setup_16  ;; 0x4007d002
        STRB     R0,[R12, #+0]
...
??LPLD_PLL_Setup_17:
        ...
        ANDS     R4,R4,#0xF0000
        ORRS     R0,R4,R0
        LDR.N    R4,??LPLD_PLL_Setup_16+0x4  ;; 0x40048044
        STR      R0,[R4, #+0]
...
//  373   MCG->C2 = MCG_C2_RANGE0(1);
        MOVS     R0,#+16
        LDR.N    R4,??LPLD_PLL_Setup_16+0x8  ;; 0x40064001
        STRB     R0,[R4, #+0]
...
//  376   MCG->C1 = MCG_C1_CLKS(2) | MCG_C1_FRDIV(5);
        MOVS     R0,#+168
        LDR.N    R4,??LPLD_PLL_Setup_16+0xC  ;; 0x40064000
        STRB     R0,[R4, #+0]
//  377   while (MCG->S & MCG_S_IREFST_MASK){}; // 等待参考时钟清零
??LPLD_PLL_Setup_18:
        LDR.N    R0,??LPLD_PLL_Setup_16+0x10  ;; 0x40064006
        LDRB     R0,[R0, #+0]
        LSLS     R0,R0,#+27
        BMI.N    ??LPLD_PLL_Setup_18
...
//  383   MCG->C6 |= MCG_C6_CME0_MASK;
        LDR.N    R0,??LPLD_PLL_Setup_16+0x14  ;; 0x40064005
        LDRB     R0,[R0, #+0]
        ORRS     R0,R0,#0x20
        LDR.N    R4,??LPLD_PLL_Setup_16+0x14  ;; 0x40064005
        STRB     R0,[R4, #+0]
//  384   MCG->C5 = MCG_C5_PRDIV0(prdiv); // 设置 PLL 匹配晶振的参考分频数
        ANDS     R0,R2,#0x7
        LDR.N    R2,??LPLD_PLL_Setup_16+0x18  ;; 0x40064004
        STRB     R0,[R2, #+0]
//  385   MCG->C6 = MCG_C6_PLLS_MASK | MCG_C6_VDIV0(vdiv); 
        ANDS     R0,R3,#0x1F
        ORRS     R0,R0,#0x40
        LDR.N    R2,??LPLD_PLL_Setup_16+0x14  ;; 0x40064005
        STRB     R0,[R2, #+0]
...
//  390   MCG->C1 &= ~MCG_C1_CLKS_MASK;
        LDR.N    R0,??LPLD_PLL_Setup_16+0xC  ;; 0x40064000
        LDRB     R0,[R0, #+0]
        ANDS     R0,R0,#0x3F
        LDR.N    R2,??LPLD_PLL_Setup_16+0xC  ;; 0x40064000
        STRB     R0,[R2, #+0]
